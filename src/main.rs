...
use esp_hal::{
    clock::{ClockControl, Clocks}, gpio::{AnyInput, AnyOutput, Gpio4, Gpio5, Io, Level, Pull}, i2c::I2C, peripherals::{Peripherals, I2C0}, prelude::*, system::SystemControl, timer::timg::TimerGroup
};
...
fn main() -> ! {
    ...
    executor.run(|spawner| {
        spawner.spawn(calculate_distance(
            AnyOutput::new(io.pins.gpio6, Level::Low),
            AnyInput::new(io.pins.gpio7, Pull::None)
        )).unwrap();
        spawner.spawn(display_distance(peripherals.I2C0, io.pins.gpio4, io.pins.gpio5, clocks)).unwrap();
    });
}
